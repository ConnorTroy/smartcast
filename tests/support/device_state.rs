...
pub fn power_state(device: Arc<VizioDevice>) -> warp::reply::Json {
    ...
    {}}}"#, *device.powered_on.read().unwrap() as u32, status!(Result::Success));
...
pub fn current_input(device: Arc<VizioDevice>) -> warp::reply::Json {
    let input: &Input =
        device.input_list.get(
            &*device.current_input.read().unwrap()
        ).unwrap();
    ...
    warp::reply::json::<Value>(
        &serde_json::from_str(
            ...
            {}}}"#,
            rng.gen::<u32>(), rng.gen::<u32>(), input.hashval, input.name, status!(Result::Success))
        ).unwrap()
    )
}

pub fn list_inputs(device: Arc<VizioDevice>) -> warp::reply::Json {
    let mut rng = rand::thread_rng();
    ...
    warp::reply::json::<Value>(
        &serde_json::from_str(
            ...
            {}}}
            "#,
            rng.gen::<u32>(), rng.gen::<u32>(), rng.gen::<u32>(), items, status!(Result::Success))
        ).unwrap()
    )
}

pub fn change_input(mut val: Value, device: Arc<VizioDevice>) -> warp::reply::Json {
    ...
    let mut res = match (request.as_str(), name, hashval, device.current_input.write()) {
